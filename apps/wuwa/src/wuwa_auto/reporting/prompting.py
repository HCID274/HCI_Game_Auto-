...
summary 只能概括 confirmed_results 中实际出现的任务类型，不得从示例或其他日志阶段补入缺失任务。“讨伐强敌/Boss Challenge”与“无音区/Tacet Suppression”是不同任务，必须保留确认结果中的原名称，不能互相改名。
栏目来源必须完整对应：confirmed_results.daily 的每一项进入 daily，weekly 的每一项进入 weekly，followup 的每一项进入 followup，failed_steps 的每一项进入 issues。每个输出数组必须与对应输入数组保持相同项数和相同顺序，一条输入对应一条输出，不得合并两项、跨栏目移动、遗漏或增加输入之外的事项。这里约束信息归属与覆盖，不要求逐字照抄，可以在保持名称、状态、数字和因果关系的前提下自然改写。
若存在失败，只报告真正没有成功的步骤，并用一两句话说明日志显示的错误、恢复动作和最终结果；不要倾倒堆栈、重复同一错误或讨论无业务影响的启动警告。
日志中的“HOST DAY PHASE”是彼此独立的阶段边界；不得把较早日常阶段的错误写成后续讨伐阶段的当前原因。
...
成功参考风格：summary只概括 confirmed_results 已确认的整日结果；daily写本轮真实发生的日常事项；followup写真实存在的讨伐次数、恢复结果、声骸吸收及外围完成事实。

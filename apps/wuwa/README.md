...
UU 连接后先运行官方启动器状态机。它按已验证窗口和截图模板识别“更新/继续/安装”
主按钮，更新期间每次状态变化只点击一次并保存全屏及按钮裁剪证据，最长等待 2.5 小时；
出现“进入游戏”后启动客户端，自动跨过“点击连接”、月相观测卡领取及奖励结果页，
连续确认世界 HUD 后再交给 OK-WW。无需更新时只执行必要的进入步骤。任何失败都会
保存完整桌面和相关窗口/进程 JSON。该流程不修改官方启动器或 OK-WW 源码。可单独检查：
